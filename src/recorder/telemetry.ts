...
export function buildBookmarklet(): string {
  const src =
    `(()=>{if(window.__dtOn)return alert('demo telemetry already on');window.__dtOn=1;` +
...
/** Opens the page to demo in a popup so its bookmarklet can reach window.opener. */
export function openDemoPage(url: string): void {
  const withScheme = /^https?:\/\//i.test(url) ? url : `https://${url}`;
  window.open(withScheme, 'demo-page', 'width=1280,height=800');
}
